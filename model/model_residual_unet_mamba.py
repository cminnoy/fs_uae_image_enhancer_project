...
class ExportableMambaBlock(nn.Module):
    ...
    def forward(self, x):
        if self.export_mode:
            return self._onnx_compatible_scan(x)
        return self.mamba(x)

    def _onnx_compatible_scan(self, u):
        orig_dtype = u.dtype
        d_inner = self.mamba.d_inner
        L = u.shape[1]

        # 1. Projections
        xz = self.mamba.in_proj(u)
        x, z = xz.chunk(2, dim=-1)

        x = x.transpose(1, 2)
        x = self.mamba.conv1d(x)[:, :, :L]
        x = x.transpose(1, 2)
        x = F.silu(x)

        # 2. SSM Parameters
        x_db = self.mamba.x_proj(x)
        dt, B_ssm, C_ssm = torch.split(x_db, [self.mamba.dt_rank, self.d_state, self.d_state], dim=-1)

        # FIX: Remove manual bias addition. The linear layer dt_proj already handles it.
        dt = F.softplus(self.mamba.dt_proj(dt))

        # 3. Stable Discretization & Scan
        dt = dt.float().unsqueeze(-1)
        A = -torch.exp(self.mamba.A_log.float()).view(1, 1, d_inner, self.d_state)
        
        # log(bar_A) = dt * A
        log_bar_A = dt * A
        
        # STABILITY: For L=424k, we must prevent exp(-cum_log_A) from hitting infinity.
        # We use the log-sum-exp trick logic applied to the linear recurrence.
        cum_log_A = torch.cumsum(log_bar_A, dim=1)
        
        # We clamp the decay to -20 (approx 2e-9) to maintain enough context 
        # while preventing numerical collapse in the cumulative product.
        cum_log_A_stable = torch.clamp(cum_log_A, min=-30.0, max=0.0)
        
        bar_B_x = (dt * B_ssm.float().unsqueeze(2)) * x.float().unsqueeze(-1)

        # h_t = exp(cum_log_A) * cumsum(bar_B_x * exp(-cum_log_A))
        # Adding 1e-12 to the divisor for safety
        exp_A = torch.exp(cum_log_A_stable)
        h = exp_A * torch.cumsum(bar_B_x * torch.exp(-cum_log_A_stable), dim=1)

        # 4. Final Projections
        y = (h @ C_ssm.float().unsqueeze(2).transpose(-1, -2)).squeeze(-1)
        
        D = self.mamba.D.float().view(1, 1, -1)
        y = (y + (x.float() * D)) * F.silu(z.float())

        return self.mamba.out_proj(y.to(orig_dtype))

class MambaCrossScan(nn.Module):
    def __init__(self, channels):
        super().__init__()
        # We share the logic but keep separate weights for H and V paths
        self.mamba_h = ExportableMambaBlock(channels)
        self.mamba_v = ExportableMambaBlock(channels)
    ...
    def forward(self, x):
        ...
        # 1. Horizontal Path (Scanline order)
        x_h = x.permute(0, 2, 3, 1).reshape(B, H * W, C)
        y_h = self.mamba_h(x_h).view(B, H, W, C).permute(0, 3, 1, 2)

        # 2. Vertical Path (Column-major order)
        # This allows the model to "see" vertical dithering patterns instantly
        x_v = x.permute(0, 3, 2, 1).reshape(B, W * H, C)
        y_v = self.mamba_v(x_v).view(B, W, H, C).permute(0, 3, 2, 1)

        return self.norm(res + y_h + y_v)
...
class ResidualUNet(nn.Module):
    def __init__(self, input_channels=3, output_channels=3,
                 ...
                 lores_only=False, verbose=False, use_mamba=False):
        super().__init__()
        ...
        self.use_mamba_bottleneck = use_mamba
        ...
        if unet_depth > 0:
            ...
            if self.use_mamba_bottleneck:
                self.bottleneck = MambaCrossScan(bottleneck_ch)
            else:
...
def get_model(name: str = 'light', lores_only: bool = False, verbose: bool = False):
    ...
    if name == 'light':
        ...
        return ResidualUNet(
            ...
            max_channels=128,
            k_paths_head=5 if lores_only else 3,
            k_paths_bottleneck=0,  # Not used when MambaBottleneck is active
            internal_block_channels_ratio=2.0 if lores_only else 1.5,
            lores_only=lores_only,
            verbose=verbose,
            use_mamba=True
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_type', type=str, required=True, choices=['light', 'heavy', 'mamba_light'], help='Type of model: light, heavy, mamba_light')
